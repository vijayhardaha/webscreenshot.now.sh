...
export class HeaderComponent implements OnInit {
  ...
  formatUrl() {
    let url = this.url.value.toLowerCase();
  ...
  handleError(error: HttpErrorResponse) {
    ...
    swal({
      title: "Oops!",
      text: "Something is wrong with the process, Please try again later.",
      icon: "info",
    });
  ...
  showScreenShot(args: any) {
    this.getScreenShot(args).subscribe((data: any) => {
